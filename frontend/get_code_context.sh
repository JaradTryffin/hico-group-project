...
# List of directories to look for inside src folder
...
if [ ! -d "${project_dir}/src" ]; then
  echo "Error: src directory not found in the project root."
  exit 1
fi

# Call the recursive function for each specified directory in the src directory
for dir in "${directories[@]}"; do
  if [ -d "${project_dir}/src/${dir}" ]; then
    read_files "${project_dir}/src/${dir}"
  fi
done
